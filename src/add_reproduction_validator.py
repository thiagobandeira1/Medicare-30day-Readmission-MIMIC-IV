...
Loads results/v17_reference_metrics.json (original capstone report's published numbers) and
...
The original original capstone report's stability estimate was **0.7956 ± 0.0026** (5-fold patient-grouped CV). Our current run is a single train/val/test split, so we should land in or just below this range; verdicts of ~ are expected for a single-split point estimate.
"""
