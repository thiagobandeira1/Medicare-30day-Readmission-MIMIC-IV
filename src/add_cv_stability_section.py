...
The single-split point estimate above (§10.1 / §10.3b) gives a clear, peer-review-defensible test AUROC, but it is one realisation. To compare directly against the V17 report's stability estimate of **0.7956 ± 0.0026 (5-fold CV)**, we ran a matched 5-fold patient-grouped cross-validation on the same V7 (50-feature) parquet, with a 10% inner-validation slice carved from each fold's training portion for early stopping (so no fold ever leaks its own test partition into model selection).

Loaded from `results/cv5_summary.json` (produced by `src/run_5fold_cv.py`). The grey band on the figure marks the V17 published stability range; per-fold AUROCs (dots) and the ± std interval are plotted for every GBM family plus the scipy-optimised blend.
"""
...
if not cv_path.exists():
    ...
    print("=" * 78)
    print(f"5-FOLD CV STABILITY  (V17 report reference: 0.7956 ± 0.0026)")
    print("=" * 78)
    print(f"{'Model':<10} {'Mean':>8} {'Std':>8}  per-fold AUROCs")
    for m, s in cv["models"].items():
        folds_str = "  ".join(f"{a:.4f}" for a in s["fold_aucs"])
        in_band = "[in V17 band]" if 0.7956 - 0.0026 <= s["mean_auroc"] <= 0.7956 + 0.0026 else ""
