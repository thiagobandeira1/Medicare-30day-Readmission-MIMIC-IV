"""5-fold patient-grouped CV stability analysis on the true original capstone dataset (V7).

Matches the protocol the original capstone report used to produce its
...
Outputs:
 ...
 figures/cv5_stability.png box-plot / strip-plot vs original capstone reference band
...
def _orchestrate(models, n_folds):
 ...
 print()
 print("=" * 78)
 print(f"5-FOLD CV STABILITY (original capstone: 0.7956 ± 0.0026)")
 print("=" * 78)
...
def _build_stability_figure(summary):
 ...
 # original capstone reference band: 0.7956 ± 0.0026 (5-fold CV)
 ref_mean, ref_std = 0.7956, 0.0026
 ax.axhspan(ref_mean - ref_std, ref_mean + ref_std, color=PALETTE["gray"],
 alpha=0.20, label=f"original capstone 5-fold-CV band ({ref_mean:.4f} ± {ref_std:.4f})")
 ...
 "(dots = per-fold; error bar = ± std; band = originally published)",
